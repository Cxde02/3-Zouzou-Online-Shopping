	...
	while ($row = fetch_array($query)) {
		...
		if ($count < 8) {
		    ...
		    <div class="col-4">
		    	...
		    	<p style="color: #8d0000; padding-top: 5px; font-family: 'courier new' ">Rs {$row['product_price']}</p>
	...
	while ($row = fetch_array($query)) {
		...
		$product = <<<DELIMETER
		    <div class="col-4">
		    	...
		    	<h4><a href="product_detail.php?id={$row['product_id']}">{$row['product_title']}</a></h4>
		    	<p style="color: red; font-family: 'courier new'; padding-top: 10px">Rs {$row['product_price']}</p>
		    	<p>{$row['product_short_desc']}</p>
		        <a class="btn" href="cart.php?add={$row['product_id']}">Add to cart</a>
		    </div>
		DELIMETER;
...
function add_user() {
    ...
    if(isset($_POST['add_user'])) {
        ...
        if (mysqli_num_rows($query) > 0) {
            ...
            redirect("login_page.php");
            set_message ("USER ALREADY EXISTS! PLEASE LOGIN");
        }
        else{
            ...
            confirm($query);
            redirect("login_page.php");
            set_message("USER CREATED. LOGIN TO CONTINUE");
...
function get_category_products() {
	...
	<div class="col-4">
		    	<a href="product_detail.php?id={$row['product_id']}"><img width='100' src="uploads/{$row['product_image']}"></a>
		    	<h4><a href="product_detail.php?id={$row['product_id']}">{$row['product_title']}</a></h4>
		    	<p style="color: red;">Rs {$row['product_price']}</p>
		    	<p>{$row['product_short_desc']}</p>
